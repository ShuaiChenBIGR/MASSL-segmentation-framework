...
def getLesionDetection(testImage, resultImage):
    ...
    lResult = sitk.Multiply(testImage, resultImage)

    ccTestArray = sitk.GetArrayFromImage(testImage)
    lResultArray = sitk.GetArrayFromImage(lResult)

    # recall = (number of detected WMH) / (number of true WMH)
    nWMH = np.sum(ccTestArray)
    if nWMH == 0:
        recall = 1.0
    else:
        recall = float(np.sum(lResultArray)) / nWMH

    # Connected components of results, to determine number of detected lesions
    lTest = sitk.Multiply(resultImage, testImage)

    ccResultArray = sitk.GetArrayFromImage(resultImage)
    lTestArray = sitk.GetArrayFromImage(lTest)

    # precision = (number of detections that intersect with WMH) / (number of all detections)
    nDetections = np.sum(ccResultArray)
    if nDetections == 0:
        precision = 1.0
    else:
        precision = float(np.sum(lTestArray)) / nDetections
